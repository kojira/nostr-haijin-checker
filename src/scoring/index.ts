 ...
 *  - streak（連続実稼働ストリーク）は **任意**（ストリークを計測したときだけ）の長期系
 *    シグナル。控えめな固定重み 0.12 で、短期の活発さを支配しない範囲で総合に効く。
 *    longTerm と違い観測信頼度では割り引かない（取得済みイベントから連続日を直接導出済みのため）。
 ...
 * @param streak ストリーク（連続実稼働日数）。取得済みイベント（メインの取得結果）から
 *            deriveStreak で導いた連続実稼働日数。**渡された場合は**「連続実稼働」シグナル
 *            （長期軸・重み 0.12）として総合スコアに加点し、長期軸サブスコアにも反映する。
 *            連続日数が長いほどスコアが上がる（約 60 日で頭打ちの飽和加点）。ストリークを
 *            計測しない（null）場合は合算・正規化・signals のいずれにも現れない。
...
export function scoreEvents(
  ...
  for (const n of historyNotes(fetchMeta)) notes.push(n);
  // ストリーク（連続実稼働日数）は取得済みイベントから導出した結果（加点済み＝上の streakSignal）。
...
/**
 * ストリーク（連続実稼働日数）から注意書きを作る。
 *
 * ストリークは **メインの取得（適応的タイムウィンドウ取得）で集めたイベントから導出**した
 * 連続日数である。別経路の追加取得は行わず、日ごとに「その日に投稿が 1 件でもあるか」を
 * 取得済みイベントから集計して数える。得られた連続日数は「連続実稼働」シグナル
 * （長期軸・重み 12%）として総合スコアに加点される。導出元・加点の効き方を明示する。
 */
export function streakNotes(streak: StreakInfo | null): string[] {
  ...
  if (streak.currentStreakDays === 0) {
    out.push(
      "連続実稼働日数（ストリーク）は 0 日です（最新の実稼働日が見つからない、または直近に活動がありません）。" +
        "ストリークは取得済みイベントから日ごとに「その日に投稿が 1 件でもあるか」を集計して数えます（別経路の追加取得はしません）。" +
  ...
  out.push(
    `連続実稼働日数（ストリーク）: ${streak.currentStreakDays} 日（最新の実稼働日 ${last} / ${status}）。` +
      "これはメインの取得で集めたイベントから、日ごとに「その日に投稿が 1 件でもあるか」を集計して数えたものです" +
      "（別経路の追加取得は行いません）。" +
      "この連続日数は「連続実稼働」シグナル（長期軸・重み 12%・約 60 日で頭打ちの飽和加点）として総合スコアに反映され、連続日数が長いほどスコアが上がります。",
  );
  if (streak.truncated) {
    out.push(
      "取得が履歴を掘り切れていない（各種上限・タイムアウト等）ため、連続実稼働日数は観測できた範囲での下限です。" +
